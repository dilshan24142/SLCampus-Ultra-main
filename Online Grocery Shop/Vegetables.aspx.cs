...
public partial class Vegetables : System.Web.UI.Page
{
    ...
    protected void btnSubmitPost_Click(object sender, EventArgs e)
    {
        ...
        {
            string query = "INSERT INTO BlogPosts2 (UserID, Title, Content, ImageUrl, VideoUrl) VALUES (@UserID, @Title, @Content, @ImageUrl, @VideoUrl)";
    ...
    private void LoadBlogPosts(string searchQuery = "")
    {
        using (SqlConnection conn = new SqlConnection(connectionString))
        {
            string query = "SELECT * FROM BlogPosts2 WHERE Title LIKE @SearchQuery OR Content LIKE @SearchQuery";
            SqlCommand cmd = new SqlCommand(query, conn);
